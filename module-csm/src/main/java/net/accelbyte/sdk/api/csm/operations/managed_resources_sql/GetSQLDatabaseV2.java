 ...
 *
 * <p>`resourceStatus` field - indicates the SQL cluster status: - `updating` : The cluster is being
 * modified and is not yet accessible (e.g., updating min/max ACU). - `failed` : The cluster failed
 * to provision or is in an error state and not accessible. - `stopping` : The cluster is in the
 ...
 * rebooting. - `creating` : The cluster or instance is being created and is not yet accessible. -
 ...
 * `unknown` : The cluster status is not recognized - `configuring-replica` : The cluster is
 * configuring replica instances for multi-AZ deployment. - `available` : The cluster is accessible.
 */
@Getter
@Setter
