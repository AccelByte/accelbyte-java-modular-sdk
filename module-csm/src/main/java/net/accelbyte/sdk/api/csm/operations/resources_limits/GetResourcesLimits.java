...
public class GetResourcesLimits extends Operation {
  ...
  @Deprecated
  public GetResourcesLimits(String customBasePath, String namespace) {
  ...
  public GetResourcesLimitsOpResponse parseResponse(
      int code, String contentType, InputStream payload) throws HttpResponseException, IOException {
    final GetResourcesLimitsOpResponse response = new GetResourcesLimitsOpResponse();
    ...
    if (code == 204) {
      ...
      response.setData(new ApimodelCSMAppLimitsResponse().createFromJson(json));
      response.setSuccess(true);
    } else if (code == 401) {
      final String json = Helper.convertInputStreamToString(payload);
      ...
      final String json = Helper.convertInputStreamToString(payload);
      response.setError402(new ResponseErrorResponse().createFromJson(json));
      response.setError(response.getError402().translateToApiError());
    } else if (code == 403) {
      final String json = Helper.convertInputStreamToString(payload);
  ...
  /*
  public ApimodelCSMAppLimitsResponse parseResponse(int code, String contentType, InputStream payload) throws HttpResponseException, IOException {
      if(code != 200){
          final String json = Helper.convertInputStreamToString(payload);
