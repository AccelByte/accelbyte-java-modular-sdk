 ...
 * TokenGrantV3
 *
 * <p>Supports grant type: 1. Grant Type == `authorization_code`: It generates the user token by
 * given the authorization code which generated in "/iam/v3/authenticate" API response. It should
 * also pass in the redirect_uri, which should be the same as generating the authorization code
 * request. 2. Grant Type == `password`: The grant type to use for authenticating a user, whether
 * it's by email / username and password combination or through platform. 3. Grant Type ==
 * `refresh_token`: Used to get a new access token for a valid refresh token. 4. Grant Type ==
 ...
 * empty if the token is generated from the client credential - **roles** (deprecated). The subâs
 * roles. It is empty if the token is generated from the client credential - **namespace_roles**.
 * The subâs roles scoped to namespace. Improvement from roles, which make the role scoped to
 * specific namespace instead of global to publisher namespace - **permissions**. The sub or audâ
 ...
 * obtained from /bans.
 *
 * <p>## Device Cookie Validation _**For grant type "password" only**_ Device Cookie is used to
 * protect the user account from brute force login attack, [more detail from OWASP. It will read the
 * device cookie from cookie **Auth-Trust-Id**. If device cookie not found, it will generate a new
 * one and sets it into the response body **auth_trust_id** on successful login.
 *
 * <p>## Track Login History It will track login history to detect suspicious login activity.
 * Provide **Device-Id** (alphanumeric) in the request header, otherwise it will be set to
 * "unknown". Align with General Data Protection Regulation in Europe, user login history will be
 * kept within 28 days by default"
 *
 * <p>## 2FA remember device To remember device for 2FA, the request should provide cookie:
 * device_token or header: Device-Token
 *
 * <p>## Login Queue _**For grant type "code" and "password" only**_ When the Login Queue is enabled
 * and at capacity, this API returns a 401 Unauthorized response, with the queue ticket included in
 * the response body.
 *
 * <p>## Legal eligibility check If it is a user token request and user hasn't accepted required
 * legal policy (if any), the field `is_comply` will be false in response and response token will
 * have no permission.
 */
@Getter
@Setter
public class TokenGrantV3 extends Operation {
  ...
  public TokenGrantV3OpResponse parseResponse(int code, String contentType, InputStream payload)
      throws HttpResponseException, IOException {
    final TokenGrantV3OpResponse response = new TokenGrantV3OpResponse();
