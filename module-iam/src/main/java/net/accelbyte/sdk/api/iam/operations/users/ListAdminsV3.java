 ...
 * ListAdminsV3
 *
 * <p>List all users that has admin role (role that has admin_role attribute set to true). Behavior
 * : - if query parameter is defined, searches users whose email address and display name match with
 * the query - if roleId parameter is defined, searches users that have the defined roleId - if
 * startDate and endDate parameters is defined, searches users which created on the certain date
 * range - if startDate parameter is defined, searches users that created start from the defined
 * date - if endDate parameter is defined, searches users that created until the defined date
 *
 ...
 *
 * <p>Returns all admin from all namespace when called from publisher namespace. When not called
 * from publisher namespace, returns all admin from the path namespace.
 */
@Getter
@Setter
public class ListAdminsV3 extends Operation {
  ...
  public ListAdminsV3OpResponse parseResponse(int code, String contentType, InputStream payload)
      throws HttpResponseException, IOException {
    final ListAdminsV3OpResponse response = new ListAdminsV3OpResponse();
