...
public class GetEventsGameTelemetryV1AdminNamespacesNamespaceEventsGet extends Operation {
    ...
    */
    ...
    public Map<String, List<String>> getQueryParams(){
        ...
        queryParams.put("eventPayload", this.eventPayload == null ? null : Arrays.asList(this.eventPayload));
    ...
    public GetEventsGameTelemetryV1AdminNamespacesNamespaceEventsGetOpResponse parseResponse(int code, String contentType, InputStream payload) throws HttpResponseException, IOException {
        final GetEventsGameTelemetryV1AdminNamespacesNamespaceEventsGetOpResponse response = new GetEventsGameTelemetryV1AdminNamespacesNamespaceEventsGetOpResponse();
        ...
        if (code == 204) {
            ...
            response.setData(new PagedResponseGetNamespaceEventResponse().createFromJson(json));
            response.setSuccess(true);
        }
        else if (code == 400) {
            final String json = Helper.convertInputStreamToString(payload);
            response.setError400(new BaseErrorResponse().createFromJson(json));
            response.setError(response.getError400().translateToApiError());
        }
        else if (code == 422) {
            final String json = Helper.convertInputStreamToString(payload);
            response.setError422(new HTTPValidationError().createFromJson(json));
    ...
    /*
    public PagedResponseGetNamespaceEventResponse parseResponse(int code, String contentType, InputStream payload) throws HttpResponseException, IOException {
        if(code != 200){
            final String json = Helper.convertInputStreamToString(payload);
